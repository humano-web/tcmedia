        ...
        <div className={`w-full relative min-h-screen px-12 2xl:pt-52 pb-12 flex flex-col 2xl:inline-flex justify-start items-center bg-center bg-cover snap-start snap-always`} style={{ backgroundImage: main ? `url(${main})` : undefined }}>
            ...
            {style === "row" ?
                <div className={`w-full tc${color}-tbg p-8 absolute left-0 top-0 h-full flex flex-row gap-6 `}>
                    <motion.div className={`w-1/2 text-tc${color} h-full pt-38 flex justify-start ${lposy === "top" ? "items-start" : lposy === "bottom" ? "items-end" : "items-center"} ${ltext ? "text-left" : "text-right"}`}>
                    ...
                    <div className={`w-1/2 text-tc${color} min-h-full pt-38 flex ${rposy === "top" ? "items-start" : rposy === "bottom" ? "items-end" : "items-center"} ${rtext ? "text-right" : "text-left"}`}>
                ...
                :
                <div className={`w-full tc${color}-tbg p-8 absolute left-0 top-0 h-full flex flex-col gap-6 ${tplace === "top" ? "justify-start" : tplace === "bottom" ? "justify-end" : "justify-center"} ${tplace === "left" ? "items-start" : tplace === "right" ? "items-end" : "items-center"}`}>
                    <motion.div className={` flex w-1/2 h-1/4 text-tc${color} text-7xl 2xl:text-9xl font-black font-['Montserrat_Alternates'] leading-24 tracking-tighter ${lposy === "top" ? "items-start" : lposy === "bottom" ? "items-end" : "items-center"} ${lposx === "left" ? "text-left" : lposx === "right" ? "text-right" : "text-center"}`}
                        initial={{ opacity: 0, [lani === "top" || lani === "bottom" ? "y" : "x"]: lani === "left" || lani === "top" ? -50 : 50 }}
                        whileInView={{ opacity: 1, [lani === "top" || lani === "bottom" ? "y" : "x"]: 0 }}
                        transition={{ duration: 0.5 }}
                        viewport={{ amount: 0.8 }}
                    >{title}</motion.div>
                    <motion.div className={`flex w-1/2 h-1/4 text-tc${color} text-lg 2xl:text-3xl font-medium font-['Montserrat'] leading-4 2xl:leading-8 tracking-normal ${rposy === "top" ? "items-start" : rposy === "bottom" ? "items-end" : "items-center"} ${rposx === "left" ? "text-left" : rposx === "right" ? "text-right" : "text-center"}`}
                        initial={{ opacity: 0, [lani === "top" || lani === "bottom" ? "y" : "x"]: lani === "left" || lani === "top" ? 50 : -50 }}
                        whileInView={{ opacity: 1, [lani === "top" || lani === "bottom" ? "y" : "x"]: 0 }}
                        transition={{ duration: 0.5 }}
                        viewport={{ amount: 0.8 }}
