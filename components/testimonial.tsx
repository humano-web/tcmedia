...
const Testimonial = ({ text = "", author = "", color = "" }) => {
  ...
  return (
    <div className="w-full min-h-screen h-96 px-4 py-8 2xl:inline-flex 2xl:flex-row justify-center items-center gap-7 overflow-hidden snap-start snap-always relative">
      ...
      >
        <div className="2xl:flex-1 2xl:inline-flex gap-1 2xl:flex-col justify-center items-start px-16 py-8"
        >
          <q ref={quoteRef} className={`text-tc${color}-dark 2xl:text-4xl font-bold font-['Montserrat'] leading-10 tracking-tighter opacity-0`} dangerouslySetInnerHTML={{ __html: text || "Comment here" }}
          >
          </q>
        </div>
      </div>
      <div ref={authorRef} className={`w-1/2 text-right justify-end text-tc${color}-dark 2xl:text-3xl font-extrabold font-['Montserrat'] leading-3 pt-8 absolute bottom-20 right-80 -translate-y-1/2 px-16 py-8 mix-blend-multiply -z-10 opacity-0`}
      >
        — {author}
      </div>
    </div>
